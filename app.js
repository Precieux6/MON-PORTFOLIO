/* =====================================================================
PORTFOLIO JAVASCRIPT - VERSION CORRIGÉE (8 PROJETS)
==================================================================== */
...
const projectsData = {
  ...
  "3": {
    title: "Campagne Instagram 12 Carrousels — Vocal Coach Factory",
  ...
  "4": {
    ...
    deliverables: [
      "50+ articles optimisés",
      "Dossier Google Drive structuré",
  ...
  "6": {
    ...
    deliverables: [
      "Supports multicanaux",
      "Charte éditoriale B2B",
...
function initProjects() {
  ...
  const requiredModalElements = [
    ...
    modalDescP2,
    modalSkillsTags,
    modalDeliverablesTags
  ];

  if (requiredModalElements.some((element) => !element)) {
    return;
  }

  const openModal = (projectId) => {
    const data = projectsData[normalizeId(projectId)];

    if (!data) {
      return;
    }

    modalImg.src = cleanText(data.image);
    modalImg.alt = cleanText(data.title);

    modalCategory.textContent = cleanText(data.category);
    modalMetric.textContent = cleanText(data.metric);
    modalTitle.textContent = cleanText(data.title);
    modalDescP1.textContent = cleanText(data.desc1);
    modalDescP2.textContent = cleanText(data.desc2);
    ...
    skills.forEach((skill) => {
      ...
      tag.textContent = cleanText(skill);
    ...
    deliverables.forEach((item) => {
      const tag = document.createElement("span");
      tag.className = "modal-tag";
      tag.textContent = cleanText(item);
  ...
  projectCards.forEach((card) => {
    card.addEventListener("click", (event) => {
      if (event.target.closest && event.target.closest(".project-link")) {
        return;
      }
...
/* --- MODULE 5 : FORMULAIRE DE CONTACT --- */

function initContactForm() {
  /*
    Le formulaire est configuré pour fonctionner avec Web3Forms
    directement depuis le HTML.
    Aucune logique JavaScript supplémentaire n'est nécessaire ici.
  */
}
